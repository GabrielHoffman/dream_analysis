...
```{r}
countMatrix = countMatrix[1:2000,]
```

```{r run.permutations, message=TRUE}
simResultsList = lapply(1:2, function(iter){

  set.seed(iter)

  # simulate phenotype so replicates from an individual have same value
  ...
  res = lapply( 2:3, function(n_reps){
    ...
    # res = lapply( c(5, 10, 20, 40, 60, 90), function(n_indivs_total){ 
    res = lapply( c(5), function(n_indivs_total){ 
      ...
      # FMT         
      fit_FMT.vc = eBayesFMT( fitSat, info.tmp, "indiv", "VC" )
      fit_FMT.ws = eBayesFMT( fitSat, info.tmp, "indiv", "WS" )
      ...
      timeMethods$macau = system.time({
        ...
        # K[1:5, 1:5]
        macau_fit <- macau2(countMatrix.tmp[isexpr,], info.tmp$phenotype, dsgn, RelatednessMatrix=K, fit.model="PMM",numCore=1, filtering=FALSE, verbose=TRUE)
      }) 
      ...
      df = data.frame(EnsID = rownames(fit_FMT.vc), 
        FMT.vc = topTable(fit_FMT.vc, coef='phenotype', sort.by="none", number=Inf)$P.Value, 
        stringsAsFactors=FALSE)
      de_res_p = merge( de_res_p, df, by="EnsID", all=FALSE )

      df = data.frame(EnsID = rownames(fit_FMT.vc), 
        FMT.ws = topTable(fit_FMT.ws, coef='phenotype', sort.by="none", number=Inf)$P.Value, 
        stringsAsFactors=FALSE)
      de_res_p = merge( de_res_p, df, by="EnsID", all=FALSE )
