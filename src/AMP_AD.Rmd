...
# Cannot include Run or other covariates with out making the model singlar
design = model.matrix( ~ 0 + Region + Region:bbscore + LIMS  + `Sex (final)` + RIN + PMI + Age + Apo1  + Apo2, metadata[idx,]) #+ Run
 # + rRNA.rate
...
AD_interaction = c( "RegionBM44:bbscore", 'RegionBM10:bbscore', 'RegionBM22:bbscore', 'RegionBM36:bbscore')

# # Fit the dream model on each gene
# # Apply the contrast matrix L for the hypothesis test  
# # By default, uses the Satterthwaite approximation for the hypothesis test
fitmm = dream( vobjMM[,idx], form, metadata[idx,])
```

# Compare -log10 p from dream and duplicateCorrelation
```{r compare_braak}
p1 = topTable(fitDupCor, coef=AD_interaction, number=Inf, sort.by="none")$P.Value
p2 = topTable(fitmm, coef=AD_interaction, number=Inf, sort.by="none")$P.Value
...
a = topTable(fitDupCor, coef=AD_interaction, number=Inf, sort.by="none")['ENSG00000137285','P.Value']
b = topTable(fitmm, coef=AD_interaction, number=Inf, sort.by="none")['ENSG00000137285','P.Value']

fig = plotCompareP( p1, p2, vp$individualIdentifier, dupcor$consensus)
png("test.png")      
fig + geom_point(aes(x=-log10(a), y= -log10(b)), size=2, color="red")
dev.off()
...
res = topTable(fitDupCor, coef=AD_interaction, number=Inf) 
enrich_dupCor_camera = get_enrich_order( res ) 
   
res = topTable(fitmm, coef=AD_interaction, number=Inf)  
...
write.table(topTable(fitDupCor, coef=AD_interaction, number=Inf), file=file, sep="\t", quote=FALSE)

file = "./AMP_AD_dream.tsv"
write.table(topTable(fitmm, coef=AD_interaction, number=Inf), file=file, sep="\t", quote=FALSE)
...
# ensGene = rownames(topTable(fitDupCor, coef=AD_interaction))[2]
ensGene = 'ENSG00000137285'
# topTable(fitDupCor, coef=AD_interaction)[ensGene,]
# topTable(fitmm, , coef=AD_interaction, number=Inf)[ensGene,]
