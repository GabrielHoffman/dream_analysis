...
ggplot(dfroll, aes(delta, resCor_CMC)) +  geom_point() + theme_bw(19) + theme(aspect.ratio=1) + xlab(xlab) + ylab(ylab) + geom_smooth(method="loess", span=1, se=FALSE) + ylim(ylim) + xlim(xlim) + annotate("text", x = xlim[1]*.9, y = ylim[2]*.9, label = paste0('rho = ', format(resCor_CMC$estimate, digits=3), "\np < ",resCor_CMC$p.value)) + ggtitle("CMC")

ggplot(dfroll, aes(delta, resCor_DGN)) +  geom_point() + theme_bw(19) + theme(aspect.ratio=1) + xlab(xlab) + ylab(ylab) + geom_smooth(method="loess", span=1, se=FALSE) + ylim(ylim) + xlim(xlim) + annotate("text", x = xlim[1]*.9, y = ylim[2]*.9, label = paste0('rho = ', format(resCor_DGN$estimate, digits=3), "\np < ",resCor_DGN$p.value)) + ggtitle("DGN")
```
