...
dfr2 = fread(getFileLocation(synGet('syn17083748')))
...
df_cor = with(resCor_CMC, c(dataset='AMP_AD', eQTL = 'CMC', estimate, p=p.value))
df_cor = rbind(df_cor, 
  with(resCor_DGN, c(dataset='AMP_AD', eQTL = 'DGN', estimate, p=p.value)))
...
ggplot(dfroll, aes(delta, resCor_CMC)) +  geom_point() + theme_bw(19) + theme(aspect.ratio=1) + xlab(xlab) + ylab(ylab) + geom_smooth(method="loess", span=1, se=FALSE) + ylim(ylim) + xlim(xlim) + annotate("text", x = xlim[1]*.9, y = ylim[2]*.9, label = paste0('rho = ', format(resCor$estimate, digits=3), "\np < ",resCor$p.value)) + ggtitle("CMC")

ggplot(dfroll, aes(delta, resCor_DGN)) +  geom_point() + theme_bw(19) + theme(aspect.ratio=1) + xlab(xlab) + ylab(ylab) + geom_smooth(method="loess", span=1, se=FALSE) + ylim(ylim) + xlim(xlim) + annotate("text", x = xlim[1]*.9, y = ylim[2]*.9, label = paste0('rho = ', format(resCor$estimate, digits=3), "\np < ",resCor$p.value)) + ggtitle("DGN")
```
