...
rmarkdown::render("~/scripts/dream/ampad_v4.Rmd", output_dir='./', intermediates_dir='./')
--->
...
geneCounts = readRDS("/sc/orga/projects/va-biobank/resources/DATA-SETS/AMP-AD/expression/featureCounts/AMP-AD.featurecounts.exon.geneID.rds")

# format counts
geneInfo = geneCounts[,1:6]
# geneCounts = geneCounts[,-c(1:6)]
# rownames(geneCounts) = geneInfo$Geneid

# # restrict to individuals in both files
# indivs = intersect(info$Lib, colnames(geneCounts))
# info = info[info$Lib %in% indivs,]
# geneCounts = geneCounts[,colnames(geneCounts) %in% indivs]
```

```{r read.geneCounts}
files = dir('/sc/orga/scratch/wangm05/synapse/','AMP-AD_MSBB_MSSM_BM_.*.raw_counts.tsv', full.names=TRUE)
...
```{r metadata}
# df_rna_covariates = fread('/hpc/users/hoffmg01/work/dev_dream/ampad/MSBB_RNAseq_covariates.csv')
df_rna_covariates = fread(getFileLocation(synGet( 'syn6100548' ))
# df_clinical = fread('/hpc/users/hoffmg01/work/dev_dream/ampad/MSBB_clinical.csv')
df_clinical = fread(getFileLocation(synGet( 'syn6101474' ))
# df_wes = fread('/hpc/users/hoffmg01/work/dev_dream/ampad/MSBB_WES_covariates.csv')
df_wes = fread(getFileLocation(synGet( 'syn6101472' ))

# sampleInfo = fread('/hpc/users/hoffmg01/work/dev_dream/ampad/MSBB_RNAseq.WES.WGS_sample_QC_info.csv')
...
```{r DE.counts_braak}
table(topTable(fitDupCor, number=Inf)$adj.P.Val < 0.05)
table(topTable(fitmm, number=Inf)$adj.P.Val < 0.05)     
```
...
fig1 = plotStratify( Expression ~ Indiv, GE, colorBy = GE$bbscore, main= ensGene) + theme(aspect.ratio=1, legend.position="bottom")
...
```{}
topTable(fitDupCor,number=Inf)[ensGene,]
topTable(fitmm,number=Inf)[ensGene,]
```

```{r plotStratify1}
ensGene = rownames(topTable(fitmm))[1]
