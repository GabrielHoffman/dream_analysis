...
# Number of DE genes from DupCor 
```{r}
sum(topTable(fitDupCor, number=Inf)$adj.P.Val < 0.05)
```

# Number of DE genes from dream 
```{r}
...
fig1 = plotStratify( Expression ~ Indiv, GE, colorBy = GE$bbscore, main= ensGene, legend=FALSE) + theme(aspect.ratio=1, legend.position="right") + scale_color_continuous(name = "Braak stage")

plotStratify( GE, 'Indiv', 'Expression', colorBy = GE$bbscore, main= ensGene, legend=FALSE) + theme(aspect.ratio=1, legend.position="right") + scale_color_continuous(name = "Braak stage")
