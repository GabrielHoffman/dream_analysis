...
resCor = with(dfroll, cor.test(delta, r2mu, method="spearman") )
...
ggplot(dfroll, aes(delta, r2mu)) +  geom_point() + theme_bw(12) + theme(aspect.ratio=1) + xlab(xlab) + ylab(ylab) + geom_smooth(method="loess", span=1, se=FALSE) + ylim(ylim) + xlim(xlim) + annotate("text", x = xlim[1]*1.1, y = ylim[2]*.9, label = paste0('rho = ', format(resCor$estimate, digits=3), "\np < ",resCor$p.value))
