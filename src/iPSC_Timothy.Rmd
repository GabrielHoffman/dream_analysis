...
## DE versus eQTL
```{r DE.vs.eQTL, echo=FALSE, fig.height=12, fig.width=12}
...
figList = foreach( ds = names(dupCorList) ) %do% {
  ...
  df_cor = with(resCor_CMC, c(dataset='iPSC_Timothy', eQTL = 'CMC', estimate, p=p.value))
  df_cor = rbind(df_cor, 
  with(resCor_DGN, c(dataset='iPSC_Timothy', eQTL = 'DGN', estimate, p=p.value)))
  ...
  fig1 = ggplot(dfroll, aes(delta, CMC_R2_mu)) +  geom_point() + theme_bw(19) + theme(aspect.ratio=1) + xlab(xlab) + ylab(ylab) + geom_smooth(method="loess", span=1, se=FALSE) + ylim(ylim) + xlim(xlim) + annotate("text", x = xlim[1]*.9, y = ylim[2]*.9, label = paste0('rho = ', format(resCor_CMC$estimate, digits=3), "\np < ",resCor_CMC$p.value)) + ggtitle(paste("CMC", ds))

  ylim = c(0, max(dfroll$DGN_R2_mu))
  fig2 = ggplot(dfroll, aes(delta, DGN_R2_mu)) +  geom_point() + theme_bw(19) + theme(aspect.ratio=1) + xlab(xlab) + ylab(ylab) + geom_smooth(method="loess", span=1, se=FALSE) + ylim(ylim) + xlim(xlim) + annotate("text", x = xlim[1]*.9, y = ylim[2]*.9, label = paste0('rho = ', format(resCor_DGN$estimate, digits=3), "\np < ",resCor_DGN$p.value)) + ggtitle(paste("DGN", ds))
...
foreach( ds = names(enrichList_dream_tstat) ) %do% {
  ...
  # dupCor  
  file = paste0("iPSC_Timothy", gsub(" ", "_", ds),"_DupCor.tsv")
  tab = topTable( dupCorList[[ds]]$fitDupCor, coef="genotype control", number=Inf )
  write.table(tab, file=file, sep="\t", quote=FALSE)

  # dream
  file = paste0("iPSC_Timothy", gsub(" ", "_", ds),"_dream.tsv")
  write.table(topTable(dreamList[[ds]], number=Inf ), file=file, sep="\t", quote=FALSE)

  # dupCor
  file = paste0("iPSC_Timothy", gsub(" ", "_", ds),"_DupCor_enrichment.tsv")
  write.table(enrichList_dp_tstat[[ds]], file=file, sep="\t", quote=FALSE)

  # dream
  file = paste0("iPSC_Timothy", gsub(" ", "_", ds),"_dream_enrichment.tsv")
  write.table(enrichList_dream_tstat[[ds]], file=file, sep="\t", quote=FALSE)
}
```

# Publication plot
```{r publ.plot, fig.height=20, fig.width=20}
# neurons at rest
gset = list()
