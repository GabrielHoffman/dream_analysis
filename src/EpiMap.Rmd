...
```{r dream3}
fitDreamStd = foreach( ct = ctArray ) %do% {
  form = ~ 0 + BrainRegion + Individual_ID
  L = getContrast( vobj[,idx], form, metadata[idx,], c('BrainRegionDLPFC', 'BrainRegionACC'))
  idx = which(metadata$CellType == ct)
  fit = dream( vobj[,idx], form, metadata[idx,], L)  
 eBayes(fit)   
}
names(fitDreamStd) = ctArray
```


# Compare -log10 p from dream and duplicateCorrelation
```{r compare2, fig.width=15}
figCompare = foreach( ct = ctArray ) %do% {

  p1 = topTable(fitDreamStd[[ct]],number=Inf, sort.by="none")$P.Value
  p2 = topTable(fitDream[[ct]], number=Inf, sort.by="none")$P.Value
