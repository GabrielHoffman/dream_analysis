...
L = getContrast( vobj, form, metadata, c('CellTypeNeuN+:BrainRegionDLPFC', 'CellTypeNeuN+:BrainRegionACC'))

```
```{r dream2}
fit = dream( vobj[1:10000,], form, metadata, L)
fit = eBayes(fit)   
```
...
fitDupCor <- lmFit(vobj[1:10000,], design, block=metadata$Individual_ID, correlation=dupcor$consensus)
...
plotCompareP( p1, p2, vp$Individual_ID, dupcor$consensus)
```
