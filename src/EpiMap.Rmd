...
if( ! exists("cl") ){
  cl <- parallel::makeForkCluster(nthreads, outfile='')
...
fitDream = foreach( ct = ctArray ) %do% {
  ...
  idx = which(metadata$CellType == ct)
  fit = dream( vobj[1:100,idx], form, metadata[idx,], L)  
 eBayes(fit)   
}
...
fitDupCorList = foreach( ct = ctArray ) %do% {
  ...
  design = model.matrix( ~ 0 + BrainRegion + Sex, metadata[idx,])
   
  # Estimate linear mixed model with a single variance component
  # Fit the model for each gene, 
  dupcor <- duplicateCorrelation(vobj[1:100,idx],design,block=metadata$Individual_ID[idx])
    
  # But this step uses only the genome-wide average for the random effect
  fitDupCor <- lmFit(vobj[1:100,idx], design, block=metadata$Individual_ID[idx], correlation=dupcor$consensus)
...
figCompare = foreach( ct = ctArray ) %do% {
  ...
  p1 = topTable(fitDupCorList[[ct]],number=Inf, sort.by="none")$P.Value
  p2 = topTable(fitDream[[ct]], number=Inf, sort.by="none")$P.Value
